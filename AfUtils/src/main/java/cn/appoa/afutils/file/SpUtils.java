...
public class SpUtils {
    ...
    /**
     * 得到保存数据的方法，根据默认值得到保存的数据的具体类型，然后调用不同的方法获取值
     */
     ...
     * 对SharedPreference的使用做了封装，对外公布出操作的方法；
     * 注意一点，里面所有的commit操作使用了SharedPreferencesCompat.apply进行了替代，
     * 目的是尽可能的使用apply代替commit 因为commit方法是同步的，并且我们很多时候的commit操作都是UI线程中，
     * 毕竟是IO操作，尽可能异步；所以我们使用apply进行替代，apply是异步的进行写入； 但是apply相当于commit来说是new
     * API呢，为了更好的兼容，做了适配； SharedPreferencesCompat也可以给创建兼容类提供了一定的参考.
     */
