 ...
 *
 * @author Jungly jungly.ik@gmail.com 15/3/5 21:30
 */
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (textSize != -1) {
            this.mTextSize = PasswordUtil.px2sp(context, textSize);
        }

        mLineWidth = (int) ta.getDimension(R.styleable.GridPasswordView_gpvLineWidth, PasswordUtil.dp2px(getContext(), 1));
    ...
    private void inflaterViews(Context context) {
        ...
        while (index < mPasswordLength) {
            View dividerView = inflater.inflate(R.layout.gridpasswordview_divider, null);
            ...
            TextView textView = (TextView) inflater.inflate(R.layout.gridpasswordview_textview, null);
     ...
     * 获取焦点 (自动弹出键盘)
     */
    public void setForceInputViewGetFocus() {
    ...
    /**
     * Interface definition for a callback to be invoked when the password changed
     * or is at the maximum length.
     */
     ...
     * @param enabled
     */
    public void setCanInput(boolean enabled) {
        canInput = enabled;
        for (int i = 0; i < mViewArr.length; i++) {
            mViewArr[i].setEnabled(enabled);
        }
    }
}
