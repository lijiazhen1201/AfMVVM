...
package cn.appoa.afui.widget.layout.rc.helper;
...
public class RCHelper {
    ...
    public void initAttrs(Context context, AttributeSet attrs) {
        ...
        mRatio = ta.getFloat(R.styleable.RCAttrs_ratio, 0.0f);
        if (mRatio <= 0.0f) {
            float ratio_width = ta.getFloat(R.styleable.RCAttrs_ratio_width, 1.0f);
            float ratio_height = ta.getFloat(R.styleable.RCAttrs_ratio_height, 0.0f);
            if (ratio_width > 0 && ratio_height >= 0) {
                mRatio = ratio_height / ratio_width;
            }
        }
        ta.recycle();
