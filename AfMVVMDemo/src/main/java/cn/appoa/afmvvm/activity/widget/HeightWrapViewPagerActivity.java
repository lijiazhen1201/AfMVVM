...
import cn.appoa.afmvvm.databinding.ActivityHeightWrapViewPagerBinding;
 ...
 * 自动适应图片高度的ViewPager
 */
public class HeightWrapViewPagerActivity extends BaseActivity<ActivityHeightWrapViewPagerBinding, TitleBarViewModel> {

    @Override
    public int initContent(Bundle savedInstanceState) {
        return R.layout.activity_height_wrap_view_pager;
    }
    ...
    public void initViewObservable() {
        viewModel.setDefaultTitleBar(R.drawable.back_black, "HeightWrapViewPager", true);
    }
    ...
    public void initData() {
        ...
        if (imgs != null && imgs.size() > 0) {
            ...
            binding.tvPage.setText("1/" + images.size());
            binding.viewPager.setBanner(images);
            binding.viewPager.setOnPageSelectedListener(new HeightWrapViewPager.OnPageSelectedListener() {
                ...
                public void onPageSelected(int position, ArrayList<String> images) {
                    //页面选中时
                    binding.tvPage.setText(position + 1 + "/" + images.size());
                }
            });
        }
    }
