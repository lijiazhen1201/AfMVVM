...
public class VideoRangeBar extends View {
    ...
    // 默认值
    public static int DEFAULT_TICK_COUNT = 3;
    ...
    public VideoRangeBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void rangeBarInit(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.VideoRangeBar, 0, 0);
        try {

            // Sets the values of the user-defined attributes based on the XML
            // attributes.
            final Integer tickCount = ta.getInteger(R.styleable.VideoRangeBar_tickCount, DEFAULT_TICK_COUNT);
            ...
            mTickHeightDP = ta.getDimension(R.styleable.VideoRangeBar_tickHeight, DEFAULT_TICK_HEIGHT_DP);
            mBarWeight = ta.getDimension(R.styleable.VideoRangeBar_barWeight, DEFAULT_BAR_WEIGHT_PX);
            mBarColor = ta.getColor(R.styleable.VideoRangeBar_barColor, DEFAULT_BAR_COLOR);
            mConnectingLineWeight = ta.getDimension(R.styleable.VideoRangeBar_connectingLineWeight,
                    DEFAULT_CONNECTING_LINE_WEIGHT_PX);
            mConnectingLineColor = ta.getColor(R.styleable.VideoRangeBar_connectingLineColor,
                    DEFAULT_CONNECTING_LINE_COLOR);
            mThumbRadiusDP = ta.getDimension(R.styleable.VideoRangeBar_thumbRadius, DEFAULT_THUMB_RADIUS_DP);
            mThumbImageNormal = ta.getResourceId(R.styleable.VideoRangeBar_thumbImageNormal,
                    DEFAULT_THUMB_IMAGE_NORMAL);
            mThumbImagePressed = ta.getResourceId(R.styleable.VideoRangeBar_thumbImagePressed,
                    DEFAULT_THUMB_IMAGE_PRESSED);
            mThumbColorNormal = ta.getColor(R.styleable.VideoRangeBar_thumbColorNormal, DEFAULT_THUMB_COLOR_NORMAL);
            mThumbColorPressed = ta.getColor(R.styleable.VideoRangeBar_thumbColorPressed,
    ...
    public static interface OnRangeBarChangeListener {
         void onIndexChangeListener(VideoRangeBar rangeBar, int leftThumbIndex, int rightThumbIndex);
    }
}
